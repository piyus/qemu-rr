...
static uint32_t apic_mem_readl(void *opaque, target_phys_addr_t addr)
{
    ...
    if (replaying_fp) {
      val = hw_replay(RR_ENTRY_TYPE_APIC);
    } else {
    ...
    if (recording_fp) {
      hw_record(val, RR_ENTRY_TYPE_APIC);
    }
...
static int apic_init1(SysBusDevice *dev)
{
    ...
    apic_io_memory = cpu_register_io_memory(apic_mem_read,
                                            apic_mem_write, NULL);
    register_deterministic_func_pointer(apic_mem_readb);
    register_deterministic_func_pointer(apic_mem_readw);
    register_deterministic_func_pointer(apic_mem_readl);
